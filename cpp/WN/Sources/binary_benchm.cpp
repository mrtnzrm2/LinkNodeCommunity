 ...
 */
int nb_distinct_elements(deque<int> degree_list) {
    ...
    for (int i = 0; i < degree_list.size(); i++) {
        ...
        if (s.find(degree_list[i]) == s.end()) {
            s.insert(degree_list[i]);
            res++;
        }
    }
...
int build_subgraph(deque<set<int> > & E, const deque<int> & nodes, const deque<int> & degrees) {
	...
	// int top_level_stopper = 0;

	// while (top_level_stopper <= 10000) {
    ...
    int random_mate;
		...
		if (nb_distinct_elements(degree_list)== 1) {
			continue;
		}
	...
	for (int i=0; i<multiple_edge.size(); i++) {
		...
		while (true) {
			...
			if (nb_distinct_elements(nodes) == 1) {
				continue;
			}
			else {
