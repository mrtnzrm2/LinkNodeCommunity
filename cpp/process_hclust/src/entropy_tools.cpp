...
bool find_end(std::map<std::string, vertex_properties> &v, const std::string &root,  const char* key) {
  for (std::set<std::string>::iterator roo = v[root].post_key.begin(); roo != v[root].post_key.end(); ++roo) {
