...
#include "fastcluster_dm.h"
#include "fastcluster_R_dm.h"
...
//
// Hierarchical clustering using memory-efficient single-linkage (memory_efficient_MST_linkage_core)
//
// Input arguments:
//   n        = number of observables
//   edges    = vector of edge_struct, each encoding a pair of neighboring nodes and their dissimilarity (distance)
//   max_dist = maximum allowed distance for merging disconnected components
// Output arguments:
//   merge    = allocated (n-1)x2 matrix (2*(n-1) array) for storing result.
//              Format: merge is a flat array of length 2*(n-1), where for each step i (0-based),
//              merge[i] and merge[i + n - 1] contain the indices of the merged nodes.
//              Indices follow R hclust convention: observables are indexed from 1, and negative values
//              indicate leaf nodes (atoms).
//   height   = allocated array of length (n-1), where height[i] is the distance at merge step i.
//              Each entry corresponds to the dissimilarity at which the clusters were merged.
// Return code:
//   0 = ok
//   1 = error
//
...
int hclust_fast_edgelist(int n, const std::vector<edge_struct>& edges, double max_dist, int* merge, double* height) {
  // run the memory-efficient MST linkage core
  cluster_result Z2(n-1);

  memory_efficient_MST_linkage_core(n, edges, max_dist, Z2);

  // Output the result in R hclust format
  int* order = new int[n];
  generate_R_dendrogram<false>(merge, height, order, Z2, n);
...
// Construct the linkage matrix in R format from merge/height arrays
std::vector<std::vector<double>> make_linkage_matrix(const int* merge, const double* height, int n) {
    ...
    // Initial cluster sizes for leaves
    for (int i = 0; i < n; ++i) {
        cluster_sizes[i] = 1;
    }
    // node_size for merged clusters
    for (int i = 0; i < n-1; ++i) {
        int idx1 = (merge[i] < 0) ? -merge[i] - 1 : merge[i] + n - 1;
        int idx2 = (merge[i + n - 1] < 0) ? -merge[i + n - 1] - 1 : merge[i + n - 1] + n - 1;
        int new_cluster = n + i;
        cluster_sizes[new_cluster] = cluster_sizes[idx1] + cluster_sizes[idx2];
        linkage[i][0] = merge[i];
        linkage[i][1] = merge[i + n - 1];
        linkage[i][2] = height[i];
        linkage[i][3] = cluster_sizes[new_cluster];
    }
    return linkage;
}