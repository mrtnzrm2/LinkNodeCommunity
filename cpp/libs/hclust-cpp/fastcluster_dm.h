/**
 * @file fastcluster_dm.h
 * @brief Fast hierarchical clustering routines for R and Python.
 *
 * This header provides efficient implementations of hierarchical, agglomerative clustering algorithms,
 * including single, complete, average, weighted, Ward, centroid, and median linkage methods.
 * It supports both the "stored matrix approach" (pairwise dissimilarities) and the "stored data approach" (vector data).
 *
 * Key features:
 * - Handles infinity values correctly and throws exceptions for NaN distance values.
 * - Provides memory-efficient algorithms for large datasets.
 * - Includes utility classes for cluster results, doubly linked lists, union-find, and binary min-heaps.
 * - Supports GCC visibility attributes to minimize exported symbols in shared objects.
 *
 * Algorithms implemented:
 * - MST_linkage_core: Single linkage clustering using minimum spanning tree (Rohlf, 1973).
 * - NN_chain_core: Nearest-neighbor-chain algorithm for various linkage methods (Murtagh, 1985).
 * - generic_linkage: Generic algorithm for all distance update formulas (Müllner, 2011).
 * - MST_linkage_core_vector: Single linkage clustering for vector data.
 * - generic_linkage_vector: Generic algorithm for vector data (Ward, centroid, median).
 * - generic_linkage_vector_alternative: Alternative nearest neighbor update scheme for vector data.
 *
 * Exception handling:
 * - Throws nan_error for NaN values in distance computations.
 * - Throws fenv_error (if FE_INVALID is defined) for floating-point environment errors.
 *
 * Compatibility:
 * - Handles platform-specific issues with floating-point environment headers.
 * - Provides compatibility for older Microsoft Visual Studio versions.
 *
 * @author Daniel Müllner (2011) <http://danifold.net>
 * @author Jorge Martinez Armas (lines 184-191 and 351-424: MeMST_linkage_core addition)
 *
 * @copyright Copyright © 2011 Daniel Müllner
 * @copyright Copyright © 2024 Jorge Martinez Armas (lines 184-191 and 351-424: MeMST_linkage_core addition)
 */
...
//   edge_struct(t_index l1, t_index l2, t_float d)
//     : link1(l1), link2(l2), dist(d) {}
// };
...
public:
  ...
  // void print_parent_first20() const {
  //   std::cout << "parent[0..19]: ";
  //   for (int i = 0; i < 20 && i < nextparent; ++i) {
  //     std::cout << parent[i] << " ";
  //   }
  //   std::cout << std::endl;
  // }
...
// Priority queue comparator: min-heap by distance, tie-break by node index
struct pq_cmp {
    bool operator()(const std::pair<t_index, t_float>& a, const std::pair<t_index, t_float>& b) const {
        if (a.second != b.second)
            return a.second > b.second; // smaller distance higher priority
        return a.first > b.first; // if equal, smaller index higher priority
    }
};

/**
 * Memory-efficient MST linkage core for single-linkage hierarchical clustering using edge list.
 *
 * @param N           Number of links ( to be clustered)
 * @param edges       Vector of edge_struct, each representing neighboring node pairs and their distance
 * @param max_dist    The distance to use for merging disconnected components (user-specified maximum)
 * @param Z2          Output cluster_result (N-1 merges)
 *
 * The function produces a clustering result in the same format as MST_linkage_core.
 * All merges are encoded in Z2. For disconnected components, merges are performed at max_dist.
 */

 static void memory_efficient_MST_linkage_core(
    const t_index N,
    ...
    std::unordered_map<t_index, std::unordered_map<t_index, t_float>> edge_map;
    for (const auto& edge : edges) {
        edge_map[edge.link1 - 1][edge.link2 - 1] = edge.dist;
        edge_map[edge.link2 - 1][edge.link1 - 1] = edge.dist;
    }

    // Active node set
    std::set<t_index> active_nodes;
    for (t_index i = 0; i < N; ++i)
        active_nodes.insert(i);

    // Distance array for tracking shortest edges to active nodes
    std::vector<t_float> d(N, max_dist);

    // Initialize d from node 0 using edge_map
    for (t_index neighbor = 1; neighbor < N; ++neighbor) {
        auto it = edge_map[0].find(neighbor);
        if (it != edge_map[0].end()) {
            d[neighbor] = std::min(d[neighbor], it->second);
        }
        // else d[neighbor] remains at max_dist
    }

    // First merge: find the nearest neighbor to node 0
    t_index idx2 = -1;
    t_float min_dist = max_dist;
    ...
    Z2.append(0, idx2, min_dist);
    active_nodes.erase(0);

    // Main MST loop (N-2 more merges)
    for (t_index step = 1; step < N-1; ++step) {
        t_index prev_node = idx2;
        active_nodes.erase(prev_node);

        // Update d for all active nodes
        for (auto node : active_nodes) {
            if (node == prev_node) continue;
            auto it = edge_map[prev_node].find(node);
            if (it != edge_map[prev_node].end()) {
                d[node] = std::min(d[node], it->second);
            }
            // else, d[node] remains as is (max_dist)
        }

        // Find next node to merge: minimal d among active nodes
        idx2 = -1;
        min_dist = max_dist;
        for (auto node : active_nodes) {
            if (d[node] < min_dist) {
        ...
        // If no active neighbor found (should not happen for connected graph), pick lowest index
        if (idx2 == -1 && !active_nodes.empty()) {
            idx2 = *active_nodes.begin();
            min_dist = d[idx2];
        }

        Z2.append(prev_node, idx2, min_dist);
    }
}
