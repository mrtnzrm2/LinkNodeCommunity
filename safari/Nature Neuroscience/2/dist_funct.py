...
def  sim_dist_bin_plot(nodes, pickle_path, ax : plt.Axes, cmap="deep"):
    ...
    axinset = ax.inset_axes([0.55, 0.14, 0.45, 0.4], transform=ax.transAxes)
    axinset.minorticks_on()
    # axinset.tick_params(top=True, right=True, which="both")

    axinset.fill_between(
        centers, iv_l_src, iv_u_src, color=cmp[0], alpha=0.2,
        edgecolor=cmp[0], linewidth=3, linestyle="--"
    )

    axinset.fill_between(
        centers, iv_l_tgt, iv_u_tgt, color=cmp[1],
        alpha=0.2, edgecolor=cmp[1], linewidth=3, linestyle="--"
    )
    ...
    axinset.text(
       1, 1.07, r"$\alpha_{-}=$" + f"{est_tgt[1]:.2f} " + "(" + r"$\pm$" + f"{alpha_tgt_err:.3f}" + ")" + "\n" +  r"$\alpha_{+}=$" + f"{est_src[1]:.2f} " + "(" + r"$\pm$" + f"{alpha_src_err:.3f}" + ")", transform=axinset.transAxes,
    ...
    plt.xlabel("Interareal distances [mm]")
