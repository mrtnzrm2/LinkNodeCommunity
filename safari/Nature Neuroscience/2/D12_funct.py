...
def  D12_bin_plot(nodes, pickle_path, ax : plt.Axes, cmap="deep"):
   ...
   #  print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
   #  print(est_tgt.conf_int()[1:])
   #  print(est_tgt.params[1])
    ...
    ax.text(
       0.1, 0.8, r"$\beta_{-}=$" + f"{est_tgt[1]:.2f} " + "(" + r"$\pm$" + f"{alpha_tgt_err:.3f}" + ") " + r"$[mm]^{-1}$" + "\n" +  r"$\beta_{+}=$" + f"{est_src[1]:.2f} " + "(" + r"$\pm$" + f"{alpha_src_err:.3f}" + ") " + r"$[mm]^{-1}$" , transform=ax.transAxes,
       ha='left', fontsize=11
    )

    ax.set_ylim(top=16.5)
