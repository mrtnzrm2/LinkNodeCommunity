...
def network_cover_plot(NET, H, ax : plt.Axes, cmap="deep", spring=False, scale=1, ang=0):
    ...
    pos[np.where(labels == "9/46d")[0][0]][0] -= 0.05
    pos[np.where(labels == "9/46d")[0][0]][1] -= 0.05

    pos[np.where(labels == "v1")[0][0]][0] += 0.07
    pos[np.where(labels == "v1")[0][0]][1] -= 0.07
    ...
    for node in G.nodes:
      ...
      a = ax.pie(
        ...
        radius=0.075,
        wedgeprops={"linewidth" : wedgewidth, "edgecolor": wedgecolor}
      )
      for i in range(len(a[0])):
        a[0][i].set_alpha(0.8)
    array_pos = np.array([list(pos[v]) for v in pos.keys()])
    ax.set_xlim(-0.1 + np.min(array_pos, axis=0)[0], np.max(array_pos, axis=0)[0] + 0.1)
    ax.set_ylim(-0.1 + np.min(array_pos, axis=0)[1], np.max(array_pos, axis=0)[1] + 0.1)
