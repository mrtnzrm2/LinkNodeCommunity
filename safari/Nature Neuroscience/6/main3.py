...
if __name__ == "__main__":
    ...
    fig = plt.figure(1, figsize=(3.54331, 5))

    gs = fig.add_gridspec(
      2, 2,  width_ratios=(4, 1), height_ratios=(1, 1),
    ...
    axb.text(
      0.0, 1.0, "b", fontsize=20, transform=axb.transAxes,
    ...
    axa = fig.add_subplot(gs[0:1, 0:1], sharex=axb)

    axa.minorticks_on() 
    axa.text(
      0.0, 1.0, "a", fontsize=20, transform=axa.transAxes,
       va='top', fontfamily='sans-serif', ha="right", weight="bold"
    )
    
    smile2_plot(NET, H, axa)

    sns.despine(ax=axa, top=True, right=True)

    ax_histy = fig.add_subplot(gs[0:1, 1:2], sharey=axa)

    hist_plot(NET, pickle_path, ax_histy)

    sns.despine(ax=ax_histy, top=True, right=True)

    plt.savefig(
      "../Publication/Nature Neuroscience/Figures/6/Figure6_8.svg", bbox_inches='tight'
    )
    plt.savefig(
      "../Publication/Nature Neuroscience/Figures/6/Figure6_8.pdf", bbox_inches='tight'
    )