...
def network_no_cover_plot(NET, H, ax : plt.Axes, cmap="deep", spring=False, scale=1, ang=0):
    ...
    partition = get_labels_from_Z(H.Z, r)
    partition = skim_partition(partition)
    partition_original = partition


    _, nocs, noc_sizes, partition  = discovery_channel["discovery_7"](H, k, partition, direction="both", index="Hellinger2")
    ...
    nx.draw_networkx_edges(
      G, pos=pos,
      edge_color="#666666",
      alpha=0.3, width=0.75, arrowsize=7,
    ...
    for node in G.nodes:
      ...
      if partition_original[node] != -1:
        ...
        a = ax.pie(
          [1], # s.t. all wedges have equal size
          center=pos[node],
          colors = ["w"],
          radius=0.075,
