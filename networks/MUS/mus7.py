...
class base:
  def __init__(self, linkage, **kwargs) -> None:
    ...
    self.csv_path = "../CSV/MUS/7d47/"
    self.distance_path = f"../CSV/MUS/7d47/{self.distance}/"
    # Labels and regions ----
...
class MUS7(base):
  def __init__(
    ...
    self.areas, self.lookup_labels = self.makeOrder()
    self.SLN, labels10 = self.get_ODR_structure()
    self.A, self.C = self.get_structure(labels10)
    self.SLN = self.SLN[self.struct_labels[:self.nodes]].reindex(self.struct_labels[:self.nodes])
    # print(self.SLN)
    self.SLN = self.SLN.to_numpy()
  ...
  def get_structure(self, labels):
    # Get structure ----
    fname = os.path.join(self.csv_path, "Mouse-Database_download_2024.csv")
    raw = pd.read_csv(fname)
    # FLN matrix
    n = len(self.areas)
    tareas = np.char.lower(np.unique(raw["Target area"].values).astype(str))
    ...
    for mouse in mouses:
        ...
        for target in targets:
            v = self.lookup_labels[target.lower()]
            ...
            for i in range(len(proc)):
                ...
                u = self.lookup_labels[src.lower()]
    ...
    self.rows = n
    struct_labels = np.char.lower(self.areas)

    zero = np.logical_or(np.isnan(FLN), np.isinf(FLN))
    FLN[zero] = 0.
    CC[zero] = 0.

    FLN = pd.DataFrame(FLN, index=struct_labels, columns=struct_labels[:m])
    CC = pd.DataFrame(CC, index=struct_labels, columns=struct_labels[:m])

    labels_check = [l for l in labels if l in tareas]
    self.struct_labels = np.array(labels_check + [l for l in struct_labels if l not in labels_check], dtype="<U21")

    FLN = FLN[labels_check].reindex(self.struct_labels)
    CC = CC[labels_check].reindex(self.struct_labels)

    self.nodes = len(labels_check)

    # print(self.struct_labels[:self.nodes], "\n")
    # print(self.struct_labels, "\n")
    # print(FLN)

    # np.savetxt(f"{self.csv_path}/labels7.csv", self.struct_labels,  fmt='%s')
    return FLN.to_numpy(), CC.to_numpy()
  ...
  def get_distance_MAP3D(self):
    ...
    for i in range(n):
        ...
        col = self.lookup_labels[tgt]
        ser = raw[tgtName]

        for j in range(n):
            src = raw['Unnamed: 0'].iloc[j]
            src = normalize_MUS_name(src)
            row = self.lookup_labels[src]
    ...
    areas = np.char.lower(self.areas)
    D = pd.DataFrame(D, index=areas, columns=areas)[self.struct_labels[:self.nodes]].reindex(self.struct_labels)
