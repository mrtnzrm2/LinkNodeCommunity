...
class MK2PreHighBeta(base):
  ...
  def get_structure(self):
    # Get structure ----
    file = pd.read_table(f"{self.csv_path}/mk2_gra_highbeta.txt", sep=" ", header=None)
    C = file.to_numpy()[:, :-1]
    self.rows = C.shape[0]
  ...
  def get_distances(self):
    file = pd.read_table(f"{self.distance_path}/mk2_precue.txt", sep="\t", header=None)
    file = file.to_numpy()
