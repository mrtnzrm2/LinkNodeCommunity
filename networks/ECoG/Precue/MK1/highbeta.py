...
class base:
  def __init__(self, linkage, **kwargs) -> None:
    ...
    self.csv_path = "../CSV/MAC/ECoG/precue/"
...
class MK1PreHighBeta(base):
  ...
  def get_structure(self):
    # Get structure ----
    file = pd.read_table(f"{self.csv_path}/mk1_gra_highbeta.txt", sep=" ", header=None)
    C = file.to_numpy()[:, :-1]
    self.rows = C.shape[0]
    self.nodes = C.shape[1]
    # Get labels ----
    labs = pd.read_table(f"{self.csv_path}/mk1_labels.txt", sep=" ", header= None)
    self.struct_labels = labs.to_numpy().ravel()
    return C
  
  def get_distances(self):
    file = pd.read_table(f"{self.distance_path}/mk1_precue.txt", sep="\t", header=None)
    file = file.to_numpy()
