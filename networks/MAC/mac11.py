...
class base:
  def __init__(self, linkage, **kwargs) -> None:
    ...
    self.csv_path = "../CSV/MAC/11d91/"
    self.distance_path = f"../CSV/MAC/11d91/{self.distance}/"
    # Labels and regions ----
...
class MAC11(base):
  ...
  def get_sln(self):
    file = pd.read_csv(f"{self.csv_path}/JCN_2013-Table.csv", header=0)
    # print(file)
    clabels = file["To"]
    clabels = np.unique(clabels)

    rlabels = file["From"]
    rlabels = np.unique(rlabels)

    rnewlabels = np.array(list(clabels) + [l for l in rlabels if l not in clabels])

    SLN = np.zeros((rnewlabels.shape[0], clabels.shape[0]))

    for i, area in enumerate(clabels):
      source_areas = file["From"].loc[file["To"] == area]
      source_area_indices = match(source_areas, rnewlabels)
      SLN[source_area_indices, i] = file["SLN(%)"].loc[file["To"] == area].to_numpy() / 100.
      
    # SLN = pd.DataFrame(SLN, index=rnewlabels, columns=clabels)
    # print(np.min(SLN[SLN > 0]))
    # SLN.to_csv(f"{self.csv_path}/sln_matrix.csv")
