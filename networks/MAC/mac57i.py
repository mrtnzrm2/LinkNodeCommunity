...
class base:
  def __init__(self, linkage, **kwargs) -> None:
    ...
    self.csv_path = "../CSV/MAC/57d106i/"
    self.distance_path = f"../CSV/MAC/57d106i/{self.distance}/"
    # Labels and regions ----
...
class MAC57i(base):
  def __init__(
    ...
    imputation="GB", iteration=0, **kwargs
  ) -> None:
  ...
  def get_structure(self):
    ...
    # Get structure ----
    FLN = pd.read_csv(
      f"{self.csv_path}/FLN_57d106_{self.imputation}{self.iteration}.csv", #"Macaque_29x91_Arithmean_DBV23.45_GB_FIN_unnormalized.csv"
      # index_col=0
    )
    ...
    np.savetxt(f"{self.csv_path}/labels57i.csv", self.struct_labels,  fmt='%s')
