...
class base:
  def __init__(self, linkage, **kwargs) -> None:
    ...
    self.csv_path = "../CSV/MAC/29d91i/"
    self.distance_path = f"../CSV/MAC/29d91i/{self.distance}/"
    # Labels and regions ----
...
class MAC29i(base):
  def __init__(
    self, linkage : str, mode : str, nlog10=True, lookup=False, imputation="GB", iteration=0,
  ...
  def get_structure(self):
    ...
    # Get structure ----
    FLN = pd.read_csv(
      f"{self.csv_path}/FLN_A_91x91_{self.imputation}{self.iteration}.csv", #"Macaque_29x91_Arithmean_DBV23.45_GB_FIN_unnormalized.csv"
      # index_col=0
    )
    ...
    # np.savetxt(f"{self.csv_path}/labels29i.csv", self.struct_labels,  fmt='%s')
