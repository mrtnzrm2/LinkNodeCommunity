...
if __name__ == "__main__":
  ...
  gt_noc = np.array([k for k, v in NET.overlap.items() if len(v) > 1])
  gt_labels = NET.labels
  gt_labels[gt_noc] = -1
  plot_h.heatmap_pure(
     0, NET.A, score = "_GT_{}".format(number_of_communities), linewidth=0.5, 
     font_size=1, labels = gt_labels, on=T
  )
  for score in opt_score:
    ...
    for ii, kr in enumerate(zip(K, R)):
      ...
      if np.nan in rlabels:
          ...
          break
      nocs, noc_covers, noc_sizes, rlabels2  = discovery_channel["discovery_7"](H, k, rlabels, direction="source", index=index)
      #Prints ----
      ...
      ## Plots ---
      plot_h.heatmap_pure(
         r, NET.A, on=T, labels = rlabels, name=f"{r}_{omega:.4f}",
         linewidth=0.5, font_size=1
      )
      plot_h.lcmap_dendro(
         k, NET.A, score="_"+score, on=T,
