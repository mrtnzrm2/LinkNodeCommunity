...
stirling_2 = lambda Nl, N : np.log((Nl/N) * (np.power(2*np.pi*Nl, 1/(2*Nl))/ np.power(2*np.pi*N, 1/(2*N))))
stirling_1 = lambda Nl, N : np.log(Nl/N)

class Hierarchical_Entropy:
  def __init__(self, Z : npt.NDArray, nodes : int, labels=[""]) -> None:
    self.N = nodes
  ...
  def get_membership_matrix(self):
    ...
    for i in np.arange(self.N - 1):
        ...
        T[(self.N+i)] = T[(nx)].union(T[(ny)])

        self.A[i + 1, :] = self.A[i, :]
        self.A[i + 1, np.array(list(T[(self.N+i)]))] = self.N + i
  ...
  def SH(self, tree : dict, Ml : dict, Sh):
    for key in tree.keys():
      ...
      Mul = len(tree[key]) - 1
      Sh[self.N - i - 1] -= Mul * stirling_2(Mul, Ml[f"L{i+1}"])
      self.SH(tree[key], Ml, Sh)

  def SV(self, Ml : dict, M, Sv):
    for key in Ml.keys():
      i = int(key[1:])
      Sv[self.N - i - 1] -= Ml[key] *  stirling_2(Ml[key], M)
  ...
  def SH_height(self, tree : dict, key, Ml : dict, maxl : int, Sh):
    ...
    if not self.is_leaf(tree[key]):
      ...
      for key2 in tree[key].keys():
        if key2 == "height" or key2 == "label": continue
        Sh[self.N - i -1] -= (tree[key]["height"] - tree[key][key2]["height"]) * stirling_2(Mul, Ml[f"L{i+1}"]["size"]) / 2
  ...
  def SV_height(self, Ml : dict, M, Sv):
    for key in Ml.keys():
      if key == "height" or key == "label": continue
      i = int(key[1:])
      Sv[self.N - i - 1] -= Ml[key]["height"] * stirling_2(Ml[key]["size"], M)
  ...
  def S(self, a):
    ...
    return np.vstack([Sh[(self.N - maxlevl):], Sv[(self.N - maxlevl):]])
  ...
  def fit(self, tree : dict, key_prev, prev_L, tL):
    ''' 
    Parameters
    ----------
    tree: dict
    ...
    key_prev: str
      Key of the previous level
    nodes_prev: set
      Set of nodes in the previous level
    L: int
      Current level in the dendrogram
    tL: int
      Current level in the hierarchy
    '''

    current_nodes, current_children, current_height = self.T[prev_L]
    if current_children != [None]:
      for c in current_children:
        ...
        self.fit(tree[key_prev], key, (c), tL + 1)
    else:
      if key_prev not in tree.keys():
        tree[key_prev] = {
          "height" : current_height,
  ...
  def Z2dict(self, Z2):
    self.tree = {}
    nodes = set(list(range(self.N)))
    L = 1
    tL = 1
    if Z2 == "short":
      self.get_membership_matrix()
      ...
      self.T = {}
      for i in range(self.N):
        self.T[(i)] = [set([i]), [None], 0]

      for i in np.arange(self.N - 1):
          nx, ny = int(self.Z[i, 0]), int(self.Z[i, 1])
          self.T[(self.N+i)] = [self.T[(nx)][0].union(self.T[(ny)][0]), [nx, ny], self.Z[i, 2]]

      self.fit(self.tree, self.root, (2*self.N-2), tL)
    else: raise ValueError("Z2 must be 'short', 'long' or 'fit'")
