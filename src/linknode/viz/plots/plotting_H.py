...
class Plot_H:
  def __init__(self, NET, H) -> None:
    ...
    # Net ----
    # if NET.root_path is not None:
    self.root_path = NET.root_path
    # else:
    #   self.root_path = ""
  ...
  def plot_newick_Rpretty(self, tree_newick, nodes, pathA, root_position = 0, weighted=False, on=True):
    if on:
      print("Plot tree in Newick format from R!!!")
      import subprocess
      # Arrange path ----
      if self.root_path != "":
        root_path = self.root_path + "/Dlink/"
      else:
        root_path = ""
      ...
      if not weighted:
        subprocess.run(["Rscript", f"{root_path}R/plot_newick_treepretty.R", tree_newick, join(plot_path, "tree_newick.png"), f"{nodes}", f"{pathA}"])
      else:
        subprocess.run(["Rscript", f"{root_path}R/plot_newick_treepretty.R", tree_newick, join(plot_path, "tree_newick_H.png"), f"{root_position}", f"{pathA}"])
    else:
      print("No tree in Newick format")

  def plot_newick_R(self, tree_newick, nodes, root_position = 0, weighted=False, threshold=False, on=True):
    if on:
      print("Plot tree in Newick format from R!!!")
      import subprocess
      # Arrange path ----
      ...
      if not weighted:
        if not threshold:
          subprocess.run(["Rscript", f"{root_path}R/plot_newick_tree_H.R", tree_newick, join(plot_path, "tree_newick.png"), f"{nodes}", "0"])
        else:
          subprocess.run(["Rscript", f"{root_path}R/plot_newick_tree_H.R", tree_newick, join(plot_path, "tree_newick.png"), f"{nodes}", f"{threshold}"])
      else:
        subprocess.run(["Rscript", f"{root_path}R/plot_newick_tree_H.R", tree_newick, join(plot_path, "tree_newick_H"), f"{root_position}"])
    else:
      print("No tree in Newick format")
    ...
    if on:
      ...
      for r in R:
        ...
        for i, i12 in enumerate(self.Z[:,:2].astype(int)):
          c1, c2 = (link_cols[x] if x > len(self.Z) else D_leaf_colors[x] for x in i12)
  ...
  def heatmap_dendro(self, r, R : npt.NDArray, score="", cmap="viridis", cbar_label="", suffix="", font_color = None, center=None, linewidth=1.5, on=True, **kwargs):
    if on:
      ...
      # labels = self.labels[den_order] # order by dendrogram not by colregion
      # labels = self.labels # order by dendrogram not by colregion
      ...
      # cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["#000000", "#6C3BAA" ,"#FDF399"])
      # cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", [DEEPPALETTE[3], DEEPPALETTE[4] , DEEPPALETTE[0]])
      ...
      sns.heatmap(
        overlay,
        cmap=matplotlib.colors.ListedColormap(["#00FF00"]),
        alpha=0.7,
        ax=ax,
        cbar=False
      )
  ...
  def lcmap_dendro(
    ...
    if on:
      ...
      # print(np.unique(dFLN.id))
      # print(np.unique(dFLN.id).shape[0])
      ...
      if -1 in dFLN:
        ...
        cmap_heatmap = [[]] * keff
        # cmap_heatmap[0] = [199/ 255.0, 0, 57/ 255.0]
        cmap_heatmap[0] = [178 / 255, 178 / 255, 178 / 255]
        cmap_heatmap[1:] = save_colors
      else:
        cmap_heatmap = sns.color_palette(cmap_name, keff)
      if not remove_labels:
        ...
        # ax.xaxis.set_major_locator(ticker.FixedLocator(np.linspace(0.5, N, 20)))
        # ax.xaxis.set_ticklabels([l for i, l in enumerate(labels) if i % 2 == 0])
        # # colors1 = [c for i, c in enumerate(colors) if i % 2 == 0]
        # # [t.set_color(c) for c, t in zip(colors1, ax.xaxis.get_ticklabels())]
        ...
        if "font_size" in kwargs.keys():
          if kwargs["font_size"] > 0:
            ax.set_xticklabels(
              ax.get_xmajorticklabels(), fontsize = kwargs["font_size"], rotation=90
            )
            ax.set_yticklabels(
              ax.get_ymajorticklabels(), fontsize = kwargs["font_size"], rotation=0
            )
            ax2.set_xticklabels(
              ax2.get_xmajorticklabels(), fontsize = kwargs["font_size"], rotation=90
            )
            ax3.set_yticklabels(
              ax3.get_ymajorticklabels(), fontsize = kwargs["font_size"], rotation=0
            )
        # if "font_size" in kwargs.keys():
        #   if kwargs["font_size"] > 0:
      ...
      fig.set_figwidth(10) 
      fig.set_figheight(9)
      ax.set_ylabel("Source", fontdict={"fontsize" : 20})
      ax.set_xlabel("Target", fontdict={"fontsize" : 20})
      fig.tight_layout()
      # Save plot ----
