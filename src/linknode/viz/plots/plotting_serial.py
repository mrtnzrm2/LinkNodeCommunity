...
class PLOT_S:
  ...
  def plot_sln_h2_corr(self, data : pd.DataFrame, on=True):
    if on:
      data_corr = data

      ff_corr_one = data_corr.loc[
        (data_corr["data"] == 1) & (data_corr["direction"] == "FF"),
        "pearsonr"
      ].to_numpy()[0]
      fb_corr_one = data_corr.loc[
        (data_corr["data"] == 1) & (data_corr["direction"] == "FB"),
        "pearsonr"
      ].to_numpy()[0]

      ff_corr_zero = data_corr.loc[
        (data_corr["data"] == 0) & (data_corr["direction"] == "FF"),
        "pearsonr"
      ]
      fb_corr_zero = data_corr.loc[
        (data_corr["data"] == 0) & (data_corr["direction"] == "FB"),
        "pearsonr"
      ]
      print(ff_corr_one)
      print(fb_corr_one)

      print(ff_corr_zero.mean())
      print(fb_corr_zero.mean())

      from scipy.stats import ttest_1samp
      print(ttest_1samp(ff_corr_zero, ff_corr_one, alternative="less"))
      print(ttest_1samp(fb_corr_zero, fb_corr_one, alternative="less"))
      ...
      plt.savefig(
        join(
          plot_path, "sln_h2_corr.png"
        ),
        dpi=300
      )
      plt.savefig(
        join(
          plot_path, "sln_h2_corr.svg"
        ),
        dpi=300
      )
      plt.close()
