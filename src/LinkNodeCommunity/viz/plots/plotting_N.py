...
class Plot_N:
  ...
  def plot_network_covers(
      ...
      ang=0, score="", direction="", cmap_name="hls", figsize=(12,12), scale=1.5, exchange_K=None, arrowsize=20,
    ...
    if exchange_K is not None:
      if not isinstance(exchange_K, (tuple, list)):
        raise ValueError("exchage_K must be a list of tuples.")
      
      for u, v in exchange_K:
        save_u = K == u
        K[K == v] = u
        K[save_u] = v
    ...
    if on:
      ...
      # Skim partition ----
      unique_clusters_id = np.unique(K)
      keff = len(unique_clusters_id)
      # Generate all the colors in the color map -----
      if -1 in unique_clusters_id:
        save_colors = sns.color_palette(cmap_name, keff - 1)
      ...
      if -1 in unique_clusters_id:
        nodes_memberships = {
          k : {"id" : [0] * keff, "size" : [0] * keff} for k in np.arange(len(K))
        }
      else:
        nodes_memberships = {
          k : {"id" : [0] * (keff+1), "size" : [0] * (keff+1)} for k in np.arange(len(K))
        }
      ...
      if "not_edges" not in kwargs.keys():
        nx.draw_networkx_edges(
          ...
          edge_color="#666666",
          alpha=0.3, width=2, arrowsize=arrowsize, connectionstyle="arc3,rad=-0.1",
      ...
      if not not_labels:
        ...
        if "modified_labels" not in kwargs.keys():
          ...
          t = nx.draw_networkx_labels(
            G, pos=pos, labels=kwargs["modified_labels"], font_color="yellow", font_size=font_size
          )
          for key in t.keys():
            t[key].set_path_effects(
            [
      ...
      plt.xlim(-0.2 + np.min(array_pos, axis=0)[0], np.max(array_pos, axis=0)[0] + 0.2)
      plt.ylim(-0.2 + np.min(array_pos, axis=0)[1], np.max(array_pos, axis=0)[1] + 0.2)
