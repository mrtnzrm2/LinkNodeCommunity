"""
Path: src/synthetic/HSF.py

Module: synthetic.HSF
Author: Jorge S. Martinez Armas

Overview:
---------
Helpers to construct deterministic hierarchical scale-free (HSF) graphs from a
seed motif. Generates labelled edgelists, node layouts, and community markers
for downstream LinkNodeCommunity analyses.

Key Components:
---------------
- HSF: grows replicated motifs, manages coordinates, and exposes adjacency and
  membership utilities.

Notes:
------
- `fit()` builds the hierarchical edgelist; `fit_matrix()` produces dense
  adjacency matrices and relabelled positions.
"""
...
class HSF:
  ...
  def to_dense_adjacency_matrix(self):
    """
